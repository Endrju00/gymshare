...
class _ActivityPageState extends State<ActivityPage> {
  ...
  int countCompleted() {
    ...
    activeWorkout.exercises?.forEach((exercise) {
      var countSeries = exerciseStats
          .where((statistic) => statistic.exerciseId == exercise.exercise.id)
          .length;
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                            ...
                            child: Text(activeWorkout.title,
                                style: const TextStyle(
                                    fontSize: 30, color: primaryTextColor))),
                        Padding(
                            padding: const EdgeInsets.only(bottom: 15.0),
                            child: Text(
                              "Completed ${countCompleted()} / ${activeWorkout.exercises?.length}",
                              style: const TextStyle(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return ExerciseTile(
                                ...
                                seriesCount: exerciseStats
                                    .where((element) =>
                                        element.exerciseId ==
                                        activeWorkout
                                            .exercises![index].exercise.id)
                                    .length,
                                completed: exerciseStats
                                        .where((element) =>
                                            element.exerciseId ==
                                            activeWorkout
                                                .exercises![index].exercise.id)
                                        .length >=
                                    activeWorkout.exercises![index].series!,
                              );
                            },
                          ),
                        )),
                                    ...
                                    child: RoundedRectangleButton(
                                        ...
                                        child: const Row(children: [
                                          ...
                                          Expanded(
                                              child: Text(
                                            "Stop",
                                            style: TextStyle(color: tertiary60),
                                          )),
                                        ]))),
                              ],
                            ))
                    ...
                    Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: DecorationImage(
                            image: NetworkImage(
                                widget.exercise.exercise.thumbnailUrl),
                            fit: BoxFit.cover,
                          ),
                        )),
                    ...
                    Expanded(
                      ...
                      children: [
                        Text(
                          ...
                          style:
                              const TextStyle(fontSize: 15.0, color: onSurface),
                        ),
                        const SizedBox(
                          height: 5.0,
                        ),
                        Text(
                          ...
                          style: const TextStyle(
                              fontSize: 12.0, color: onSurfaceVariant),
                        )
                      ],
                    )),
                    Checkbox(
...
showMyDialog(BuildContext context, String title, String msg, bool save,
  ...
  dialogValue.then((value) async {
    if (value == true) {
      ...
      await unloadWorkout()
          .then((value) => showRatingDialog(context, value, callback));
    }
  });
}
        ...
        return StatefulBuilder(builder: (context, setState) {
          return SimpleDialog(
            ...
            children: <Widget>[
              Center(
                child: Padding(
                  ...
                  child: Text(
                    ...
                    textAlign: TextAlign.start,
                    style:
                        const TextStyle(fontSize: 16, color: onSurfaceVariant),
                  ),
                ),
              ),
              Padding(
              ...
              SimpleDialogOption(
                onPressed: () {
                  Navigator.pop(context, [true, ratingValue]);
                },
                child: const Text(
                  'Ok',
                  style: TextStyle(color: primary),
