...
class ActiveExercisePage extends StatefulWidget {
  ...
  const ActiveExercisePage({super.key, required this.exerciseInWorkout});
...
class _ActiveExercisePageState extends State<ActiveExercisePage> {
  ...
  Future loadStats() async {
    ...
    if (exerciseStatsJSONString is String) {
      setState(() {
        exerciseStats.addAll(List<StatisticEntry>.from(
            jsonDecode(exerciseStatsJSONString)
                .map((w) => StatisticEntry.fromJson(w))));
      });
    }
  }
                  ...
                  children: [
                    SizedBox(
                        ...
                        child: ListView(children: [
                          Container(
                              constraints:
                                  const BoxConstraints(minHeight: 150.0),
                              ...
                              child: Padding(
                                  ...
                                  child: Text(
                                      widget.exerciseInWorkout.exercise
                                          .description!,
                    ...
                    const Padding(
                        ...
                        child: Text("Your progress",
                            style: TextStyle(
                                color: primaryTextColor, fontSize: 18.0))),
                    (widget.exerciseInWorkout.exercise.exerciseType ==
                                "With a weight" ||
                            widget.exerciseInWorkout.exercise.exerciseType ==
                    ...
                    widget.exerciseInWorkout.exercise.exerciseType ==
                            "With time"
                        ? TimerWidget(
                            timeCallback: updateTime, initialValue: time)
                        : const SizedBox(),
                    SizedBox(
                        width: 200.0,
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemCount: exerciseStats
                            .where((element) =>
                                element.exerciseId ==
                                widget.exerciseInWorkout.exercise.id)
                            .length,
                        itemBuilder: (context, index) {
                          return RecordTile(
                              recordData: exerciseStats
                                  .where((element) =>
                                      element.exerciseId ==
                                      widget.exerciseInWorkout.exercise.id)
                                  .toList()[index],
...
class TimerWidget extends StatefulWidget {
  ...
  const TimerWidget(
      {super.key, required this.timeCallback, required this.initialValue});
...
class _TimerWidgetState extends State<TimerWidget>
  ...
  Duration _getActualFromTickRawDuration(Duration rawDuration) =>
      rawDuration + _elapsed;
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      TimePickerSpinner(
        ...
        isForce2Digits: true,
        isShowSeconds: true,
        normalTextStyle: const TextStyle(fontSize: 24, color: onSurfaceVariant),
        highlightedTextStyle: const TextStyle(fontSize: 28, color: onSurface),
        onTimeChange: (time) {
          _ticker.stop();
          setState(() {
      ...
      Text("${_requested + _elapsed}".substring(0, 10),
...
class _RecordTileState extends State<RecordTile> {
  ...
  Widget build(BuildContext context) {
    return Container(
        height: 60,
        width: double.infinity,
        color: widget.idx % 2 == 0
            ? surface2.withOpacity(0.5)
            : surface3.withOpacity(0.5),
        child: Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    (widget.recordData.repeats != null)
                        ? Text(
                    ...
                    (widget.recordData.weight != null)
                        ? Text(
                            "Weight: ${widget.recordData.weight}",
                            style: const TextStyle(color: onSurface),
                          )
                        : const SizedBox(),
                    (widget.recordData.time != null)
                        ? Text(
